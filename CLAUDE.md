...
- **Recurrence (added at M8):** `task_templates` holds the recurrence rule (`rrule` RFC 5545 string + `dtstart` anchor) and the instance shape (title, notes, project, tags, plus `start_offset_days` / `due_offset_days` relative to scheduled_date). `tasks.template_id` (nullable) ties materialised instances back to a template; FK is `on delete set null` so deleting a template leaves existing rows as one-off tasks. The generator (`lib/recurrence.ts` + `useRecurrenceGenerator`) materialises a 60-day horizon on app load and again after the user changes a recurrence. Past instances are never re-created — if you delete yesterday's recurring task, it stays deleted. Editing a single instance does not propagate to future instances; edit the template via the panel for that.
...
- [x] M8 — Wishlist 1–3: (1) bold overdue/due-today task titles in calendar + project views via `isDueOrOverdue()` in `lib/dates.ts`. (2) per-project quick-add (`ProjectQuickAdd` in `ProjectView`) schedules new tasks for today. (3) Recurring tasks with the template-plus-instances model — new `task_templates` table (migration `0003_recurrence.sql`), `rrule` lib for RFC 5545 parsing, `RecurrenceEditor` in the task detail panel offering Daily / Weekly / Monthly / Custom presets with an Ends rule (never / after N / on date). Client-side generator (`useRecurrenceGenerator` + reusable `runRecurrenceGenerator`) materialises instances 60 days ahead on app load and immediately after editing the recurrence in the panel; instances inherit title/notes/project/tags from the template, and start_date/due_date offsets are derived from the source task's dates. `↻` glyph on `TaskCard` flags recurring instances. Editing a single instance affects only that row; the template is the source of truth for future occurrences.
